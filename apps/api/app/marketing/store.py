...
class MarketingStorePort(Protocol):
    ...
    async def list_due_queue(self, *, now: datetime, limit: int = 100) -> list[QueueItem]: ...
...
def demo_audience() -> list[AudienceMember]:
    return [
        AudienceMember(
            ...
            metadata={"vehicle": "2016 Honda Accord", "service": "oil change", "shop": "Apex Auto"},
        ),
        AudienceMember(
            customer_id=uuid4(),
            name="Jordan Lee",
            ...
            metadata={"vehicle": "2019 Toyota Corolla", "service": "brakes", "shop": "Apex Auto"},
        ),
        AudienceMember(
            customer_id=uuid4(),
            name="Sam Chen",
            phone="+15550102",
            metadata={"vehicle": "2014 Ford Fusion", "service": "battery", "shop": "Apex Auto"},
        ),
    ]


_DEMO_PHONES = frozenset({"+15550100", "+15550101", "+15550102", "15550100", "15550101", "15550102"})
_DEMO_NAMES = frozenset({"alex rivera", "jordan lee", "sam chen"})
...
def is_demo_member(member: AudienceMember) -> bool:
    ...
    if email in _DEMO_EMAILS:
        return True
    if name in _DEMO_NAMES and (not member.phone or phone.lstrip("+").startswith("155501")):
        return True
    return False
